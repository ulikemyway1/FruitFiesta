  ...
  constructor(hash: string) {
    ...
    fetchCategories()
      .then((responseCategories) => {
        const categories = responseCategories.body.results;
        ...
        searchParams.forEach((value, key) => {
          ...
          case "food":
            if (Array.isArray(queryArgs.filter)) {
              queryArgs.filter.push(
                `variants.attributes.is-it-food: ${value}`,
              );
            } else {
        ...
        fetchProductProjections(queryArgs)
          .then((responseProducts) => {
            this.view.appendContent(
              new Pagination(
                searchParams,
                responseProducts.body.total,
              ).getHTMLElement(),
            );

            this.view.appendContent(
              new ProductsBlockView(
                responseProducts.body.results,
              ).getHTMLElement(),
            );
